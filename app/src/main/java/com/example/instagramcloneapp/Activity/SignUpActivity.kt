...
class SignUpActivity : ComponentActivity() {
    var dataUsers: ArrayList<Users>? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        dataUsers = intent?.extras?.getParcelableArrayCompat("data", Users::class.java)
        setContent {
    ...
    private fun signUpControl(
        ...
        if (username.isNotEmpty() && password.isNotEmpty() && password2.isNotEmpty()) {
            if (password.equals(password2)) {
                ...
                Toast.makeText(context, "Girdiğiniz şifreler aynı değil.", Toast.LENGTH_SHORT)
                    .show()
            }
    ...
    fun writeData(username: String, password: String): Boolean {
        ...
        var randomId = (0..10).random()
        for (user in dataUsers!!) {
            if (user.information?.id == randomId) {
                randomId = (0..10).random()
            }
        }

        val info = Information(id = randomId, password = password, userName = username)
        val postList = ArrayList<PostItem>()
        postList.add(PostItem(id = 0, description = "", url = ""))

        val user = Users(info, postList)
        database.child(randomId.toString()).setValue(user).addOnSuccessListener() {
        }
        return true
    }
}
